...
- [ ] **B1 — Strip-10 board generation is mathematically broken** (`server/index.js` → `generateStrip10Assignments`)
...
- [ ] **B2 — Winner detection only returns/highlights the *first* matching square** (`server/index.js` → `checkCurrentWinner`; `client/src/components/BoardView.jsx` → `winningSquare` memo)
...
- [ ] **B3 — Claude image import uses a retired model** (`server/llmService.js`)
...
- [ ] **B4 — Image mime type is hardcoded to `image/png` for every upload** (`server/llmService.js`, `server/index.js` → `/api/parse-image`)
...
- [ ] **B5 — Deep links 404 on Vercel** (`vercel.json`)
...
- [ ] **B6 — Client crashes when the API returns an error payload**
...
- [ ] **B7 — Unknown `/api/*` GET routes return the SPA's `index.html` in production** (`server/index.js` static catch-all)
  API consumers get HTML instead of a JSON 404. Fix: JSON 404 for unmatched `/api/*` before the static catch-all.

- [ ] **B8 — `POST /api/boards` accepts any `type` string and never validates axes server-side**
...
- [ ] **B9 — `max_tokens: 4096` can truncate 10x10 extractions** (`server/llmService.js`)
...
- [ ] **B10 — `PUT /api/boards/:id/score` crashes on boards without `currentScore`** (`server/index.js`)
  `board.currentScore.xTeam = parsed` throws if a stored/imported board lacks the field. Fix: initialize the object when absent.

- [ ] **B11 — `PUT /api/boards/:id/squares` (bulk) has no structural validation**
...
- [ ] **B12 — Changing board type after an image import keeps stale imported squares** (`client/src/components/CreateBoard.jsx`)
...
- [ ] **B13 — Gemini API key sent as a URL query parameter** (`server/llmService.js`)
  Keys in URLs can end up in proxy/server logs. Fix: send via the `x-goog-api-key` header.

- [ ] **B14 — Missing grid squares render as clickable ghosts** (`BoardView.jsx` + `index.css`)
...
- [ ] **B15 — `loading` never resets when navigating directly between boards** (`BoardView.jsx`)
  The previous board flashes while the next one loads. Fix: reset loading state on `id` change.

- [ ] **B16 — "Example scores" for a combo all share the same x-team score** (`server/index.js` → `calculateWinningScores`)
...
- [ ] **B17 — Score inputs can't be cleared** (`BoardView.jsx`)
...
- [ ] **U1 — Prize amounts exist but period winners are never recorded**
...
- [ ] **U2 — Strip-10 digits cannot be entered or edited manually**
...
- [ ] **U3 — Image import silently "fixes" unreadable axis digits**
...
- [ ] **U4 — Tracked squares are lost on every page reload**
...
- [ ] **U5 — Zero test coverage for game-critical logic**
...
- [ ] **E1 — 10x10 grid is unusable on phones**
...
*Audit performed 2026-07-12. Checkboxes are marked as items are fixed; see git history for the corresponding changes.*
